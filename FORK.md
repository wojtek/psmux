...
bring the fork forward first. Start a fresh branch from that upstream tip and
reapply only the maintained differences above. If upstream now provides one of
them, keep upstream's implementation and remove the redundant fork patch. Do
not merge accumulated fork history over the new base.
